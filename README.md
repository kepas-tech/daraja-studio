...
Status: v0.8.0. One install = one organisation = one shortcode: Studio mirrors the Safaricom
...
- **How to use** — the manual, inside the app at `/guide` for people; the same tasks with every
  API call and the rules for agents at `/guide.md`, served only to clients that do not ask for
  HTML (agents, scripts, curl). Source: `web/src/copy/guide.ts`.
