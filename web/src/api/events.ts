...
export function useEvents(onEvent: (e: StudioEvent) => void, enabled = true, onOpen?: () => void) {
  useEffect(() => {
    ...
    for (const t of ['request.updated', 'balance.updated', 'operator.updated', 'alert', 'setup.updated', 'org.updated', 'bulk.updated']) es.addEventListener(t, handler as EventListener);
