...
describe('Setup › Environment', () => {
  ...
  it('choosing production posts production with nothing to confirm yet', async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      if (String(input) === '/api/setup/environment' && init?.method === 'POST') return new Response(null, { status: 204 });
      throw new Error(`unexpected fetch ${init?.method ?? 'GET'} ${String(input)}`);
    });
  ...
  it('shows a server error and does not call onDone', async () => {
    const fetchMock = vi.fn(async () => new Response(JSON.stringify({ error: { code: 'invalid', message: 'Something is wrong with that choice.' } }), { status: 400 }));
    vi.stubGlobal('fetch', fetchMock);
    const onDone = vi.fn();
    render(<Environment onDone={onDone} />);
    ...
    await screen.findByText('Something is wrong with that choice.');
