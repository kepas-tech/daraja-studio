...
describe('Nav', () => {
  ...
  it('keeps the rarely used destinations off the everyday menu and offers Advanced as a plain link to them', () => {
    ...
    expect(advanced.map((e) => e.key)).toEqual(['standing-orders', 'express', 'bonga', 'bulk', 'reverse', 'api-keys', 'webhooks', 'charges']);
  ...
  it('renders every nav entry with its Safaricom name', () => {
    ...
    expect(copy.nav.map((e) => e.key)).toEqual([
      'home','notifications','history','reports','stk','money-in','qr','invoices','standing-orders','express','bonga','send','contacts','bulk','sweep','schedules','approvals','reverse','api-keys','webhooks','charges','businesses','who-did-what','settings','advanced','guide','not-possible',
    ]);
  });
});
