...
describe('History', () => {
  it('lists seven at a time, pages with Next and Previous, and filters', async () => {
    const urls: string[] = [];
    ...
    expect(screen.getByText(copy.history.page(2))).toBeInTheDocument();
    expect(screen.getByRole('button', { name: copy.history.next })).toBeDisabled();
    fireEvent.click(screen.getByRole('button', { name: copy.history.previous }));
