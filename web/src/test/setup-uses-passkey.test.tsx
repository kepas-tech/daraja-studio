...
describe('Passkey', () => {
  const fill = () => {
    ...
    fireEvent.change(screen.getByLabelText(copy.setup.passkey.phone, { exact: false }), { target: { value: '0700123456' } });
  };
  ...
  it('tests the passkey against Safaricom and moves on only when it is proven', async () => {
    ...
    expect(posted).toEqual({ passkey: 'k'.repeat(64), phone: '254700123456' });
