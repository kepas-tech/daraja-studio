    ...
    tasks: [
      ...
      },
      {
        ...
        steps: [
          ...
          'Where do your paybill payments arrive today? Two honest answers. Studio receives them: turn Money in on below and Studio registers its own addresses with Safaricom. Another system receives them: nothing on Safaricom changes, and that system posts each confirmation to Studio instead — the address and a key to paste are shown under that answer.',
          'The feed takes Safaricom’s own confirmation body, untouched: the same fields the callback carries, posted to the address shown, with a key whose role is Forwarder. Make one under Advanced › API keys — that role may feed money in and do nothing else, and the key is shown once.',
          'A payment is identified by its receipt, so nothing is ever counted twice: a payment already recorded by the feed, by Studio’s own confirmation or by the hourly pull is answered as a duplicate and changes nothing. That is what makes leaving the feed on through a changeover safe.',
          'Send a test proves the whole path before the first real payment: it records one test payment through the feed, sends the very same body twice to show the receipt rule, and removes it. Nothing reaches your books, your inbox or your webhooks.',
        ...
        api: [
          ...
          { method: 'POST', path: '/api/money-in/feed', who: 'money_in.feed (an API key with the Forwarder role); Safaricom’s own confirmation body; 201 recorded, 200 duplicate' },
          { method: 'POST', path: '/api/money-in/feed/test', who: 'money_in.feed; records one test payment, proves the receipt rule, removes it' },
          { method: 'POST', path: '/api/money-in/arrival', who: 'owner; body { arrival: "studio" | "forwarder" } — the answer to where payments arrive' },
        ],
      },
      {
        key: 'businesses',
    ...
    tasks: [
      ...
      },
      {
        ...
        api: [
          ...
          { method: 'POST', path: '/api/send/name-check', who: 'send.phone; body { phone }; answers { available: true, name, paidBefore } or { available: false, reason: "not_found" | "not_enabled" | "unavailable", said, paidBefore }' },
      ...
      {
        key: 'sweep',
        title: 'Sweep-through: money paid to a business goes on to its own phone',
        safaricom: 'B2C',
        where: ['Pay out', 'Sweep-through'],
        who: 'Anyone signed in can look; the owner, or a role given the permission, sets it up',
        path: '/sweep',
        permission: 'sweep.manage',
        steps: [
          'Open Sweep-through in the menu. Each business on the paybill has a card: where its money goes, when it goes, what is kept, and what is owed to it with the payments that make the figure up.',
          'Set it up on a business: the phone number the money should reach, the timetable, and the fee. The screen says plainly what you are agreeing to: money paid to this number is sent to this phone, on this timetable, less this fee, and it happens without anyone pressing anything.',
          'The timetable is one of three: as it arrives (minutes after each payment), every day at an hour you choose, or every week on a day you choose. All three are Kenya time.',
          'The fee is yours: a percentage, a flat amount, or both, with a least and a most. It is worked out per sweep to the shilling and written on the sweep, so what was kept is never a guess later.',
          'Save asks for your password, and so does any later change to the phone or the timetable. Both are written to Who did what, with the before and the after.',
          'Stop sweeping is one press, and needs no password. The money stays owed and visible on the page, and goes the moment you start again.',
          'Each sweep is listed with its receipt. A sweep that could not go says why: a short float names the gap in shillings, and the whole sweep waits as one — never part of it. A sweep below what Safaricom will send waits and goes with the next one.',
        ],
        notes: [
        ...
        api: [
          { method: 'GET', path: '/api/sweep', who: 'signed in; every business with what it is owed, the payments behind it, and its sweeps' },
          { method: 'GET', path: '/api/sweep/:businessId', who: 'signed in; the same for one business' },
          { method: 'POST', path: '/api/sweep/:businessId', who: 'sweep.manage, password; body { destinationPhone, schedule, hour, weekday, fee: { percentBp, flatCents, floorCents, ceilingCents } }' },
          { method: 'POST', path: '/api/sweep/:businessId/stop', who: 'sweep.manage; body { stopped: true | false } — one press, no password' },
        ],
      },
      {
        key: 'approvals',
