    ...
    tasks: [
      ...
      },
      {
        ...
        steps: [
          ...
          'Type it on the keypad: six dots fill as you go, the sixth digit sends by itself, and the phone gives a small buzz. There is no button to press.',
          'Five wrong PINs lock the PIN for 15 minutes, and the line says how long is left. Your password always works instead: press "Use your password instead" on the lock screen, or on any confirmation Studio asks for.',
          'On a send, or anything else that moves money, the same keypad replaces the password box — six digits instead of a long password on a phone.',
          'Fingerprint: after a PIN opens Studio on a phone that has none set up, a card asks once whether to use the fingerprint this device already has. Say yes and the next time the fingerprint opens Studio by itself. The PIN keeps working, and money still asks for the PIN or the password, never the fingerprint alone.',
          'Organisation lists the devices with a fingerprint, each with Remove (your password or PIN is asked for first). Removing the PIN removes every fingerprint with it.',
        ...
        api: [
          ...
          { method: 'DELETE', path: '/api/auth/pin', who: 'owner, password or PIN; removes the fingerprints too' },
          ...
          { method: 'POST', path: '/api/auth/webauthn/open/options and /open/verify', who: 'signed in; the fingerprint ceremony; opens exactly what the PIN opens' },
          { method: 'POST', path: '/api/auth/webauthn/register/options and /register/verify', who: 'open session; enrols this device' },
          { method: 'GET', path: '/api/auth/webauthn/credentials', who: 'open session; the devices enrolled, never a key or a counter' },
          { method: 'POST', path: '/api/auth/webauthn/credentials/remove', who: 'open session, password or PIN; body { id }' },
        ],
      },
      {
        key: 'menu',
    ...
    tasks: [
      {
        ...
        api: [
          { method: 'GET', path: '/api/health/problems', who: 'signed in; the owner also gets the specifics behind each sentence' },
