...
export const guide: GuideSection[] = [
    ...
    tasks: [
      ...
      },
      {
        ...
        steps: [
          ...
          'Home and History come first. Then Get paid (Ask a customer to pay, Money in, QR codes, Invoices), Pay out (Send money, Contacts, Bulk send, and Waiting whenever a send needs a person), and Manage (Businesses, Settings, Advanced).',
  ...
  },
  {
    ...
    tasks: [
      ...
      {
        key: 'businesses',
        title: 'Run more than one business on one number',
        ...
        steps: [
          ...
          'From the day there are two, payers must start the account number with the business code. Tell them: pay your number, account 001007.',
          'Open a business and press Add a customer. Studio gives the customer the next number and shows the whole account number to give the payer.',
          'A payment arrives with the code and the customer number. Money in shows which customer it belongs to, and History filters by business or by one customer.',
          'A payment whose account number names no business, or a number no customer holds, waits in Money in under Payments we could not sort. Assign a business, or create the customer the payer typed, in one press.',
          'Send money and Bulk send ask which business the money is from once there are two; the last one you used is the default.',
          'Ask a customer to pay, QR codes and Invoices can pick a saved customer, and Studio fills the account reference with their account number.',
        ],
        notes: [
        ...
        api: [
          { method: 'GET', path: '/api/businesses', who: 'signed in' },
          ...
          { method: 'GET', path: '/api/businesses/:id/customers', who: 'signed in; q narrows by name or number' },
          { method: 'POST', path: '/api/businesses/:id/customers', who: 'businesses.manage; Studio mints the next number' },
          { method: 'POST', path: '/api/businesses/:id/customers/claim', who: 'businesses.manage; body { number, name }; claims the number the payer typed' },
          { method: 'PUT', path: '/api/customers/:id', who: 'businesses.manage' },
          { method: 'DELETE', path: '/api/customers/:id', who: 'businesses.manage; retires the number' },
          { method: 'POST', path: '/api/businesses/assign/:requestId', who: 'businesses.manage; body { businessId, customerId? }' },
