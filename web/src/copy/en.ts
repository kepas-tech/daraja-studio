...
export const copy = {
  qr: {
    ...
    pickCustomer: 'Pick a saved customer (optional)',
  ...
  advancedPage: {
    ...
    blurbs: {
      'standing-orders': 'A customer agrees once; Safaricom collects on a schedule you set.',
      express: 'Prompt another business\u2019s till to pay your paybill.',
      bonga: 'Let a customer pay with their Bonga points.',
  ...
  businesses: {
    ...
    customers: 'Customers',
    accountCount: (n: number) => (n === 1 ? '1 customer' : n + ' customers'),
    /** The width in use, in the owner's words. A number's length is how Studio tells levels apart. */
    numbersLine: (width: number, used: number, capacity: number) => 'Customer numbers: ' + width + (width === 1 ? ' digit' : ' digits') + ', ' + used + ' of ' + capacity + ' used',
    ...
    accountsUnder: (name: string) => 'Accounts under ' + name,
    noAccountsUnder: 'Nothing under this customer yet.',
    addAccount: 'Add an account',
    editCustomer: 'Change',
    retire: 'Retire',
    retired: 'Retired. The number is not given to anyone else.',
    ...
    pickCustomer: 'Pick a saved customer',
  ...
  invoices: {
    ...
    form: {
      ...
      pickCustomer: 'Pick a saved customer (optional)', pickCustomerHint: 'Studio fills the account reference, the name and the phone.',
  ...
  moneyIn: {
    ...
    unmatched: {
      ...
      intro: 'These payments arrived safely. The account number the payer typed does not name a business or a customer yet. Choosing who it belongs to only labels it; the money itself is untouched.',
      ...
      noSub: (reference: string, customer: string | null) => 'The account number ' + reference + (customer ? ' names ' + customer + ', but no account under them.' : ' names an account nobody holds.'),
      ...
      addCustomer: (business: string) => 'Add a customer in ' + business,
      none: 'No account yet',
  ...
  goLive: {
    ...
    number: { intro: 'The number customers pay, as on your Safaricom letter.', done: (name: string | null) => (name ? `Saved. Safaricom knows this number as ${name}.` : 'Saved.') },
    ...
    passkey: { intro: 'Only because you prompt customers’ phones. Studio sends one KES 1 prompt to your own phone as the proof; cancel it there, nothing is taken.', skip: 'You do not prompt phones, so there is no passkey to add.' },
