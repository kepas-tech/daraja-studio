...
export const copy = {
  ...
  businesses: {
    title: 'Businesses',
    intro: 'One paybill can serve more than one business. Each business gets a three-digit code, and each customer gets a number of their own, so a payment can be sorted by what the payer typed. With one business only, nothing changes: every payment belongs to it.',
    add: 'Add a business',
    ...
    customers: 'Customers',
    customerCount: (n: number) => (n === 1 ? '1 customer' : n + ' customers'),
    ...
    customerAdded: (name: string, account: string) => name + ' is customer ' + account + '. Tell them that account number.',
    editCustomer: 'Change',
    retire: 'Retire',
    ...
    pickHint: 'Studio fills in the account number for you.',
  },
  askToPay: {
  ...
  history: {
    ...
    oneCustomer: (name: string) => 'Showing one customer: ' + name,
  ...
  moneyIn: {
    ...
    unmatched: {
      ...
      noCustomer: (reference: string) => 'The account number ' + reference + ' names a business, but no customer with that number.',
      whichBusiness: 'Which business?',
      assign: 'Assign',
      assigned: 'Assigned. The payment itself is untouched.',
      createCustomer: (n: number | null, business: string) => (n === null ? 'Create the customer in ' + business : 'Create customer ' + n + ' in ' + business),
      orExisting: 'Or pick an existing customer',
