...
export const copy = {
  ...
  nav: Object.assign([
    ...
    { key: 'sweep', label: 'Sweep-through', safaricom: 'B2C', path: '/sweep', icon: 'arrow-right-circle', group: 'out', phase: 5, available: true },
    { key: 'schedules', label: 'Scheduled payments', safaricom: 'B2C and B2B', path: '/schedules', icon: 'calendar', group: 'out', phase: 5, available: true },
  ...
  /**
   * Step three of nine: sweep-through. The page that decides where a business's money goes, and
   * shows what it is owed beside the payments that make the figure up. The sentences the server
   * writes for the reasons nothing left come from the server, so the page and the money rule can
   * never say different things.
   */
  sweepPage: {
    title: 'Sweep-through',
    subtitle: 'Money paid to a business\u2019s account numbers goes on to that business\u2019s own phone, on the timetable it chose, less the fee you set. Nobody presses anything. Nothing here holds a balance: what is owed is worked out from the payments themselves, so the figure and the payments under it can never disagree.',
    empty: 'No businesses yet. Add one under Businesses first.',
    noManage: 'You need permission to decide where a business\u2019s money goes. Ask the owner.',
    owedTitle: 'Owed to this business',
    owedNothing: 'Nothing is owed right now. Every payment so far has been sent on.',
    owedFrom: (n: number) => (n === 1 ? 'From 1 payment' : 'From ' + n + ' payments'),
    paidIn: 'Paid in',
    alreadySent: 'Already sent',
    feesTaken: 'Fees taken',
    owedNow: 'Owed now',
    paymentsTitle: 'The payments it is made of',
    paymentsNone: 'No payment is waiting for this business.',
    goesTo: 'Goes to',
    noPhone: 'No phone yet, so nothing is swept',
    everyWhen: 'When',
    feeLabel: 'Fee',
    free: 'No fee',
    feeWords: (percent: string, flat: string, floor: string, ceiling: string) => {
      const parts = [percent, flat].filter(Boolean);
      const sum = parts.length === 0 ? 'No fee' : parts.join(' plus ');
      const limits = [floor ? 'never less than ' + floor : '', ceiling ? 'never more than ' + ceiling : ''].filter(Boolean);
      return sum + (limits.length > 0 ? ', ' + limits.join(', ') : '');
    },
    percent: (p: string) => p + '%',
    setUp: 'Set it up',
    change: 'Change',
    ...
    cancel: 'Cancel',
    saved: 'Saved.',
    /** The words the design asks the screen to say, once, before anybody agrees to it. */
    consentTitle: 'What this means',
    consent: 'Money paid to this number is sent to this phone, on this timetable, less this fee, and it happens without anyone pressing anything.',
    consentGiven: (at: string) => 'Agreed on ' + at + '. Changing the phone or the timetable asks for your password and is written to Who did what.',
    phone: 'Phone number the money goes to',
    phoneHint: 'A Safaricom number, written 07\u2026 or 2547\u2026.',
    when: 'When it goes',
    asItArrives: 'As it arrives',
    everyDay: 'Every day',
    everyWeek: 'Every week',
    atHour: 'At',
    onDay: 'On',
    weekdays: ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'],
    feeTitle: 'What you keep',
    feeIntro: 'A percentage, a flat amount, or both, with the least and the most it can ever be. It is taken from each sweep on its own, worked out to the shilling, and written on the sweep.',
    feePercent: 'Percentage',
    feeFlat: 'Flat amount (KES)',
    feeFloor: 'Least fee (KES)',
    feeCeiling: 'Most fee (KES)',
    feeOptional: 'Leave blank for none',
    feePreview: (owed: string, fee: string, net: string) => 'On the ' + owed + ' owed now, this takes ' + fee + ' and sends ' + net + '.',
    sweepsTitle: 'Sweeps',
    sweepsNone: 'Nothing has been swept for this business yet.',
    state: { prepared: 'Getting ready', sending: 'On its way', sent: 'Sent', failed: 'Did not go', held: 'Held' } as Record<string, string>,
    sentLine: (money: string, fee: string) => money + ' sent, ' + fee + ' kept',
    heldLine: (money: string) => money + ' held back',
    receipt: (r: string) => 'Receipt ' + r,
    madeAt: (at: string) => 'Made ' + at,
    why: 'Why',
    stillInFlight: 'Still with Safaricom. Studio is watching it.',
    awaitingApproval: 'Waiting for a second person to release it. Nothing has been sent.',
    stop: 'Stop sweeping',
    start: 'Start sweeping',
    stopped: 'Sweeping is stopped. The money stays here and is still owed.',
    stopping: 'Stop sweeping for this business? Nothing is sent while it is stopped, and nothing is lost.',
    stoppedToast: (name: string) => 'Sweeping is stopped for ' + name + '. The money is still owed.',
    startToast: (name: string) => 'Sweeping is on again for ' + name + '.',
    stepUpTitle: (name: string) => 'Change where ' + name + '\u2019s money goes',
    minimumLine: (min: string) => 'Safaricom will not send less than ' + min + ' plus its charge. Less than that waits and goes with the next sweep.',
    loadFailed: 'Studio could not read sweep-through.',
  },
  advancedPage: {
    title: 'Advanced',
  ...
  apiKeys: {
    ...
    role: 'What may it do?',
    business: 'Which business is it for?',
    businessNone: 'No business',
    businessHint: 'A collector key for a business opens an account for each of the app\u2019s own users there, so every payment names the user who paid.',
    forBusiness: (name: string) => `For ${name}`,
  ...
  request: {
    ...
    type: { b2c: 'Sent to a phone', b2b: 'Paid a business', c2b: 'Paid by a payer', invoice_payment: 'Invoice paid', ratiba: 'Standing order', express: 'Asked a business to pay', bonga: 'Paid with Bonga points', stk: 'Asked a payer to pay', reversal: 'Reversal', balance: 'Balance check', status_query: 'Payment lookup' } as Record<string, string>,
    subtype: { BusinessPayment: 'Business payment', SalaryPayment: 'Salary', PromotionPayment: 'Promotion', BusinessPayBill: 'Paybill', BusinessBuyGoods: 'Till', refresh: 'Balance refresh', lookup: 'Lookup', sweep: 'Safaricom check' } as Record<string, string>,
  ...
  send: {
    ...
    kinds: [
      ...
      { key: 'paybill', label: 'To a paybill', safaricom: 'Business PayBill', path: '/send/paybill', live: true },
      { key: 'till', label: 'To a till', safaricom: 'Business Buy Goods', path: '/send/till', live: true },
  ...
  /** B2B: pay a paybill or a till. Money leaves the Working account, where payments in land. */
  payBusiness: {
    title: { paybill: 'Pay a paybill', till: 'Pay a till' } as Record<'paybill' | 'till', string>,
    safaricom: { paybill: 'Initiate Transaction › Business PayBill', till: 'Initiate Transaction › Business Buy Goods' } as Record<'paybill' | 'till', string>,
    intro: 'Money leaves your Working account, where payments in land, and arrives at the other business.',
    number: { paybill: 'Paybill number', till: 'Till number' } as Record<'paybill' | 'till', string>,
    badNumber: 'Five to seven digits, as on the bill or the till sign.',
    account: 'Account number',
    accountHint: 'What the paybill asks you to quote: a meter, an invoice or a customer number. Up to 20 letters and numbers.',
    badAccount: 'Up to 20 letters and numbers, with no spaces.',
    amount: 'Amount (KES)', remarks: 'Note (optional)', next: 'Review', back: 'Back', pay: 'Pay',
    business: 'Which business is this money from?',
    fromContacts: { label: 'Pick a saved contact', pick: 'A name, or type the number below', saved: (name: string) => `Saved as ${name}.`, manage: 'Keep a list of businesses you pay' },
    review: {
      title: 'Check before you pay',
      checking: 'Asking Safaricom who this number belongs to…',
      name: (n: string) => `Registered to ${n}.`,
      notFound: 'Safaricom does not know this number. Check it before you pay.',
      unavailable: 'Safaricom did not say who this number belongs to. Check the number.',
      firstTime: 'You have never paid this number before. Check the number and the name before you pay.',
      account: 'Account number', noAccount: 'None',
      chargeLabel: 'Safaricom’s charge',
      charge: (amount: string) => `${amount}, taken from Working`,
      chargeNone: 'Not known for this amount.',
      debits: 'Taken from: Working account',
      balanceNow: 'Working balance now', balanceAfter: 'After this payment',
      balanceMissing: 'No balance yet. Refresh on Home to see it here.',
      short: 'Not enough in Working for this payment. Wait for more payments in, or top up in the Safaricom portal.',
      cap: (amount: string) => `This studio is capped at ${amount} per send.`,
      nameAfter: 'Safaricom’s reply names the business that was paid. It does not say the balance left.',
    },
    confirmTitle: (amount: string, to: string) => `Pay ${amount} to ${to}?`,
    duplicate: (w: string) => `You paid this already at ${w}. Pay again?`, duplicateYes: 'Yes, pay again', duplicateNo: 'No, cancel', duplicateCancelled: 'Cancelled.',
    result: { sent: 'Sent. Waiting for Safaricom…', completed: 'Paid.', failed: 'Not paid.', unknown: 'We do not know yet.', awaiting_approval: 'Waiting for a second person to approve.', payAnother: 'Pay another' } as Record<string, string>,
  },
  /** Scheduled payments: consent once, then each run goes on its own until it is stopped. */
  schedules: {
    title: 'Scheduled payments', safaricom: 'B2C and B2B, on a timetable',
    intro: 'Pay the same people on a timetable: daily casuals, weekly crews, monthly salaries, rent or a supplier. You agree once, and each payment then goes on its own until you stop it.',
    new: 'New schedule', none: 'No schedules yet.', needContacts: 'Payees come from your contacts. Save the people and businesses you pay there first.',
    state: { active: 'On', paused: 'Paused', stopped: 'Stopped', finished: 'Finished' } as Record<string, string>,
    line: (people: number, total: string, next: string | null) => `${people} ${people === 1 ? 'payee' : 'payees'}, ${total}` + (next ? `, next on ${next}` : ''),
    home: {
      line: (n: number, name: string, total: string, on: string) => `${n} payment ${n === 1 ? 'schedule is' : 'schedules are'} on. Next: ${name}, ${total}, on ${on}.`,
      paused: (n: number) => `${n} paused.`,
      seeAll: 'See them all', pause: 'Pause it', paused1: 'Paused. Nothing goes until it is turned back on.', stop: 'Stop it',
    },
    form: {
      titleNew: 'New schedule', titleEdit: (name: string) => `Change ${name}`,
      step1: 'Who gets paid, and how much', step2: 'When they are paid', step3: 'What will happen on its own',
      name: 'Name', nameHint: 'For example: Monthly salaries, Weekly loaders, Office rent.',
      addLine: 'Add a payee', payee: 'Payee', pick: 'Pick a contact', amount: 'Amount (KES)', note: 'Note (optional)', remove: 'Remove',
      phoneMin: 'At least KES 10 to a phone.', total: (t: string) => `Total each time: ${t}`,
      every: 'How often', everyOptions: { daily: 'Every day', weekly: 'Every week', fortnightly: 'Every other week', monthly: 'Every month' } as Record<string, string>,
      weekday: 'On which day', weekdays: ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'],
      dayOfMonth: 'On which date', dayOfMonthHint: 'A date a month does not have, like the 31st, pays on its last day.',
      hour: 'At what time', startOn: 'First date', endOn: 'Last date (optional)', endOnHint: 'Leave empty to keep paying until you stop it.',
      weekendMonthly: 'If the date falls on a weekend', weekendOptions: { on_day: 'Pay on the day', before: 'Pay on the Friday before', skip: 'Skip weekends' } as Record<string, string>,
      weekendDaily: 'Weekends', dailyOptions: { on_day: 'Pay every day', skip: 'Monday to Friday only' } as Record<string, string>,
      phoneCommand: 'Payments to phones are sent as', phoneCommands: { SalaryPayment: 'Salary', BusinessPayment: 'Business payment' } as Record<string, string>,
      next: 'Next', back: 'Back',
      warningTitle: 'This pays on its own',
      warning: (total: string, n: number, words: string) => `Studio will pay ${total} to ${n} ${n === 1 ? 'payee' : 'payees'} ${words}, with nobody pressing anything, and it keeps going until you stop it.`,
      firstDates: 'The first three payments', noDates: 'Those dates leave nothing to pay.',
      rules: [
        'Before each run Studio checks your balance. If it cannot cover everybody, nobody is paid and you are told how much is missing.',
        'A payment that fails is reported and never sent again on its own.',
        'Pausing or stopping takes effect before the next payment.',
      ],
      approvalNote: (amount: string) => `A payment of ${amount} or more still waits for a second person to approve it, as every send does.`,
      accept: 'I understand this pays on its own until I stop it',
      save: 'Agree and turn on', saveEdit: 'Agree and save',
      confirmTitle: (name: string) => `Turn on ${name}?`,
      created: 'Schedule on.', saved: 'Schedule saved.',
    },
    detail: {
      lines: 'Who gets paid', totalLabel: 'Total each time', timetable: 'When', consented: (who: string, at: string) => `Agreed to by ${who} on ${at}.`,
      next: 'Next payments', runs: 'Pay runs', noRuns: 'Nothing paid yet.',
      edit: 'Change it', pause: 'Pause', resume: 'Turn back on', stop: 'Stop for good',
      resumeTitle: (name: string) => `Turn ${name} back on?`, resumed: 'On again. Nothing is paid for the dates it was paused over.',
      stopTitle: 'Stop this schedule for good', stopBody: 'Nobody is paid by it again. Its history stays. To stop it, type its name.',
      stopName: 'Schedule name', stopConfirm: 'Stop it', stopped: 'Stopped. Its history stays.',
      gone: 'This contact was deleted, so this line is not paid.',
    },
    run: {
      title: (name: string, on: string) => `${name}, ${on}`,
      state: { prepared: 'Getting ready', sending: 'Sending', done: 'Paid', partly_failed: 'Some failed', failed: 'Failed', refused: 'Not sent', missed: 'Missed' } as Record<string, string>,
      lineState: { waiting: 'Not sent', sent: 'Waiting for Safaricom', paid: 'Paid', failed: 'Failed', unknown: 'Needs a check' } as Record<string, string>,
      retry: 'Try again', retryTitle: 'Send this run now?', retried: 'Sent. The lines update as Safaricom answers.',
      openPayment: 'Open the payment',
    },
  },
  contacts: {
    title: 'Contacts',
  ...
  namedNumber: {
    choose: 'Choose a name as account number',
    change: 'Change the name',
    remove: 'Stop using the name',
    current: (name: string) => 'They can also pay with account ' + name + '.',
    label: 'Name to pay with',
    hint: '3 to 12 letters and numbers, like JOHN or MARY2. Capitals do not matter.',
    check: 'Check the name',
    cancel: 'Cancel',
    free: (name: string) => name + ' is free.',
    use: (name: string) => 'Use ' + name,
    tryThese: 'These are free:',
