...
export function Reverse() {
  ...
  return (
    <>
      ...
      {step === 'review' && found && (
        <div className="max-w-lg space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <Button type="button" disabled={busy} onClick={() => { setDialogError(null); setConfirm(true); }}>{copy.reverse.button}</Button>
          </div>
      ...
      {step === 'result' && request && (
        <div className="max-w-lg space-y-4">
          <p role="status" className="text-lg">{resultCopy[request.status] ?? request.status}</p>
