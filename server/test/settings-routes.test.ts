...
describe('settings routes', () => {
  ...
  it('shortcode save requires step-up, verifies only once that environment\'s creds are saved and verified', async () => {
    const no = await request(app).put('/api/settings/environments/sandbox/shortcode').set('Cookie', cookie).set('x-csrf-token', csrf).send({ shortcode: '4052037' });
    expect(no.status).toBe(403);
    const unverified = await request(app).put('/api/settings/environments/sandbox/shortcode').set('Cookie', cookie).set('x-csrf-token', csrf).send({ shortcode: '4052037', password: 'correct horse' });
    expect(unverified.status).toBe(200);
    expect(unverified.body).toEqual({ verifiedName: null, verifyError: null });
    expect(await deps.settings.get('env.sandbox.shortcode')).toBe('4052037');
  });
  ...
  it('production mode switch needs the shortcode typed back, and never touches credentials', async () => {
    ...
    const good = await request(app).put('/api/settings/mode').set('Cookie', cookie).set('x-csrf-token', csrf).send({ environment: 'production', confirmShortcode: '4052037', password: 'correct horse' });
  ...
  it('each environment\'s creds stay independently verified across a mode switch', async () => {
    ...
    try {
      const { cookie: c2, csrf: cs2 } = await loginAsOwner(built.app, built.deps);
      await built.deps.settings.set('env.production.shortcode', '4052037');
      ...
      const switched = await request(built.app).put('/api/settings/mode').set('Cookie', c2).set('x-csrf-token', cs2).send({ environment: 'production', confirmShortcode: '4052037', password: 'correct horse' });
