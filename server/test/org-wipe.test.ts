...
describe('POST /api/org/wipe', () => {
  ...
  // By id, not by LIMIT 1: a package can make an organisation of its own, so this database is no
  // longer guaranteed to hold exactly one, and this test is about the install's own.
  const orgName = async () => (await withSystem(() => deps.db.query<{ name: string; status: string }>('SELECT name, status FROM orgs WHERE id = $1', [TEST_ORG_ID])))[0]!;

  it('refuses a wrong name, a wrong password, and anyone but the owner', async () => {
    await withSystem(() => deps.db.query(`UPDATE orgs SET name = 'ACME TRADERS', status = 'verified' WHERE id = $1`, [TEST_ORG_ID]));
  ...
  it('wipes everything but the audit trail and puts the install back to first-run setup', async () => {
    ...
    expect((await withSystem(() => deps.db.query<{ id: string }>('SELECT id FROM orgs WHERE id = $1', [orgId])))[0]!.id).toBe(orgId);
