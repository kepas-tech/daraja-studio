...
describe('callbacks', () => {
  ...
  it('failed probe marks operator failed with the Safaricom text', async () => {
    const [op] = await deps.db.query<{ id: string }>(`INSERT INTO operators(name, credential_enc, status) VALUES ('KILELO', $1, 'pending') RETURNING id`, [encrypt(deps.config.secretKey, 'c')]);
