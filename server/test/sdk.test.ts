...
describe('sdk', () => {
  ...
  it('attaches the highest-priority verified operator', async () => {
    await deps.db.query(`INSERT INTO operators(name, credential_enc, status, priority) VALUES ('KEPAS',$1,'verified',1), ('KILELO',$2,'verified',2)`,
      [encrypt(deps.config.secretKey, 'cred-kepas'), encrypt(deps.config.secretKey, 'cred-kilelo')]);
    const f = createDarajaFactory({ ...deps, secretKey: deps.config.secretKey });
    const d = await f.get();
