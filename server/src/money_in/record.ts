  ...
  const out = await deps.db.tx(async (c) => {
    ...
    const bonga = await c.query<{ id: string }>(
      `UPDATE requests SET status='completed', result_at=now(), result_source=$3, result_code='0', result_desc='Completed', receipt=$2, recipient_name=COALESCE($4, recipient_name), raw_result_json=$5::jsonb,
         business_id=COALESCE($6, business_id), customer_id=COALESCE($7, customer_id)
      ...
      [String(p.billRefNumber ?? ''), receipt, source, name, JSON.stringify(p), businessId, customerId]);
    ...
    const ins = await c.query<{ id: string }>(
      `INSERT INTO requests(type, subtype, originator_conversation_id, status, amount_cents, currency, recipient_kind, recipient_value, recipient_name,
         account_reference, payload_json, sent_at, result_at, result_source, result_code, result_desc, receipt, business_id, customer_id)
       VALUES ('c2b', $1, $2, 'completed', $3, 'KES', 'phone', $4, $5, $6, $7::jsonb, COALESCE($8::timestamptz, now()), now(), $9, '0', 'Completed', $10, $11, $12) RETURNING id`,
      [String(p.transactionType ?? '') || null, `c2b:${receipt}`, Math.round(Number(p.amount) * 100), String(p.msisdn ?? '') || null, name,
        ...
        transTimeToDate(p.transTime), source, receipt, businessId, customerId],
    );
